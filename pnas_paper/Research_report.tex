...
\subsection*{Game structure.} Three players form a team that fights a
shared boss enemy across multiple stages within a round (Fig.~1A). At the
start of each stage, every player privately selects one of three
\textbf{roles}: \textit{Fighter} (attack-focused), \textit{Tank}
(damage-blocking), or \textit{Medic} (healing). The role determines the
action the player takes on each of the two turns within the stage: a
Fighter attacks; a Tank blocks if the boss is telegraphed to attack,
otherwise attacks; a Medic heals when the team is hurt and otherwise
attacks. After each stage, every player guesses which role each of their
teammates was playing (the \emph{inference query}). A round terminates when
the team kills the boss (WIN), is wiped out (LOSE), or runs out of stages
(TIMEOUT, capped at 5 stages).

\subsection*{Stat profiles and role asymmetry.} Each player is assigned a
\textbf{stat profile} $\theta_i = (\mathrm{STR}_i,\mathrm{DEF}_i,\mathrm{SUP}_i)$
that scales the effectiveness of each role's signature action.
Profiles are sampled to break symmetry: in a profile like $\theta_i =
(4,1,1)$ the player is a much better Fighter than Tank or Medic, whereas
$(2,2,2)$ is symmetric. The team's stat profile thus suggests a
\emph{stat-optimal} role assignment, but players are free to deviate. In
some configurations the truly optimal assignment (computed by
value-iteration on the underlying Markov decision process; see Methods)
differs from the stat-suggested one---the \emph{deviate-optimal} role
assignment.

\subsection*{Round types.} A game consists of six rounds, divided between
two types. In \textbf{human rounds} all three players are real humans;
the team must coordinate a role assignment without communication. In
\textbf{bot rounds} the human plays alongside two AI bots whose roles are
fixed in advance. Crucially, the bots' roles are set to the
\emph{deviate-optimal} assignment, so the human must abandon their
stat-suggested role to fill the missing slot. Bot rounds test whether
humans can override a strong prior cue (their own stat profile) when the
joint optimum requires it.

\subsection*{Game state and observability.} The team and the boss each
have a hit-point pool. Player stats and the upcoming boss-attack
intent (whether the boss will attack or pause on the next turn) are
common knowledge; teammates' role choices and their per-turn actions are
observed only after each turn resolves. This makes role inference a
non-trivial Bayesian problem: the observed action history is the
likelihood signal, and the stat profile provides a structured prior.

\section*{Computational Models}\label{sec:models}

We model each player as a Bayesian agent that maintains a joint
probability distribution over the role-assignments of \emph{all three}
players, updates it from the observed action history, and uses it to
choose a role for the next stage. We then consider several variants that
relax different aspects of this idealised account.

\subsection*{Generative model and prior.} Let $r_i \in \{F,T,M\}$ be the
role of player $i$ and $\theta_i$ their stat profile. We define a
\emph{utility-based prior} over role assignments
\begin{equation}
P(r_1,r_2,r_3 \mid \theta_{1:3}) \propto \exp\!\left(\frac{1}{\tau_p}
\sum_{i=1}^3 \theta_i[r_i]\right),
\end{equation}
where $\theta_i[r_i]$ is the stat in $\theta_i$ corresponding to role
$r_i$ (e.g.\ STR for Fighter), and $\tau_p$ is a temperature controlling
how strongly the prior follows the stats. This captures the intuition
that, before observing any actions, teammates are more likely to play
roles their stats favour.

\subsection*{Action model and Bayesian update.} Given a role $r$, the
preferred action $a^*(r,s)$ is deterministic in the current game state
$s$: Fighters attack, Tanks block iff the boss is attacking and otherwise
attack, and Medics heal iff the team has lost HP and otherwise attack. We
allow a small amount of action noise:
\begin{equation}
P(a \mid r, s) = \begin{cases} 1-\varepsilon & a = a^*(r,s) \\
\varepsilon/2 & \text{otherwise.}\end{cases}
\end{equation}
After each turn, the joint posterior is updated by Bayes' rule using the
observed actions of all three players.

\subsection*{Bounded memory.} A perfect Bayesian observer would accumulate
all observations across stages. We instead use a \emph{drift-to-prior}
memory model: at every stage boundary, the joint posterior is pulled half
of the way back to the original prior,
$\tilde{P}_{t+1} = (1-\beta) P_t + \beta P_0$ with $\beta = 0.5$. This
captures the empirically observed loss of inference accuracy across
stages (Fig.~3) and avoids the degenerate case where the posterior
collapses to a delta after a few highly informative actions. The drift
parameter was selected by a held-out grid search against humans'
single-stage inference reports (Methods).

\subsection*{Role selection.} A player choosing a role at the next stage
must reason about (i) what their teammates will play and (ii) what action
they themselves should take. Different models differ in how they combine
these. We compare seven instantiations, organised in three families
(Table~1):

\emph{Pure inference models.}
\textbf{Bayesian-Belief} chooses each role with probability equal to the
marginal of the posterior over that player. This captures pure posterior
sampling and uses no value information.
\textbf{Bayesian-Value} computes the expected utility of each role given
the value function of the underlying game (estimated by value iteration)
and the posterior over teammates' roles, and softmaxes:
\begin{equation}
P(r_i \mid \mathcal{H}) \propto \exp\!\left(\frac{1}{\tau_v} V_i(r_i \mid
\mathcal{H})\right),
\end{equation}
where $V_i(r_i \mid \mathcal{H}) = \sum_{r_{-i}} P(r_{-i}\mid \mathcal{H})
V(r_i, r_{-i}, s)$ is the expected team value if player $i$ takes role
$r_i$ and teammates' roles are sampled from the posterior.

\emph{Hybrid (sticky) models.}
...
switching, it softmaxes over the candidates.

\emph{Posterior-sampling variants.}
\textbf{Bayesian-Walk-PS} and \textbf{Bayesian-Threshold-PS} are
walk/threshold models that use the posterior marginal in place of the
value-aware softmax, i.e.\ they do not consult the value function. A
\textbf{Mixture-PS} model averages walk-PS and threshold-PS.

\subsection*{Fitting.} We fit each model in two stages. Stage~1 fits the
inference parameters ($\tau_p, \varepsilon, $ and memory $\beta$) against
held-out human inference reports (the stage-by-stage guesses about
teammates' roles), yielding a single set of best parameters used by every
Stage~2 model. Stage~2 fits each model's role-selection parameters
($\tau_v, \varepsilon_s, \delta, w$) by maximising the Pearson
correlation between the model's predicted role-combination distribution
and the empirical distribution across humans, with cross-validation
against held-out log-likelihood. Details are in Methods.
...
quantitatively similar behaviour (Fig.~1).
