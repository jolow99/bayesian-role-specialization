...
most five \emph{stages} per round (Fig.~\ref{fig:game}). Each stage
...
role policy: a Fighter always attacks; a Tank
...
boss's hit points. Third, an \textbf{inference
...
In \textbf{human rounds} all three players are
...
\begin{figure*}[t!]
\centering
\includegraphics[width=0.72\linewidth]{fig_game_interface.png}
\caption{Screenshot of the ad-hoc role-specialization task as seen by
player P1 (mid-stage 2). Three players cooperate against a boss across
up to five stages per round. The left panel shows the shared
battlefield: the boss's STR / attack probability / HP (top right) and
each player's STR / DEF / SUP stat profile (bottom left). At the start
of each stage after the first, players are asked to infer which roles
their teammates played in the previous stage (orange panel) before
privately choosing their own role for the upcoming stage from
\{Fighter, Tank, Medic\}. Once all three players commit, two turns of
simultaneous action resolve according to the role-conditioned policy
(Fighter $\to$ attack; Tank $\to$ block when the boss attacks, else
attack; Medic $\to$ heal when the team is hurt, else attack), and the
resulting actions and HP changes appear in the Battle History sidebar
(right). A round ends in WIN (boss HP $=0$), LOSE (team HP $=0$), or
TIMEOUT (five stages elapsed).}
\label{fig:game}
\end{figure*}
...
the three components resolve simultaneously. Attacks (A) sum into the
boss in STR; blocks (B) are sub-additive in DEF (only the strongest
blocker counts); heals (H) add to team HP in SUP. Damage applies only
if the boss attacks ($\iota=1$).
