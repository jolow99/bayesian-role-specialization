"""Triple-objective × 7-model comparison on current April exports.

For each of 3 objectives (combo_r, agg_ll, mean_ll) × 7 models, fits the
model's params on the clean human team-rounds from the current exports using
...
def build_table(cells, n_records):
    lines = [
        ...
        "",
        f"Fitted on {n_records} clean human team-rounds from the current "
        "April exports (see `pipeline.load_human_team_records` — every team-round "
