"""Self-contained pipeline utilities for the current-export metric comparison.
...
Scope for this experiment:
    * All five current treatment-condition exports: 04-23, 04-27a, 04-27b,
      05-11a, 05-11b.
...
# ──────────────────────────────────────────────────────────────────────
# Data scope: all five current treatment-condition exports, clean games only
# ──────────────────────────────────────────────────────────────────────
...
EXPORT_DIRS = [EXPORTS_DIR / name for name in EXPORT_NAMES]


def discover_dropout_games(records) -> set[tuple[str, str]]:
    """Return export/game IDs that had any dropout player-round."""
    return {(pr.export_name, pr.game_id) for pr in records if pr.is_dropout}


def filter_clean_prs(records, dropout_game_ids: set[tuple[str, str]]):
    """Keep only player-rounds whose game had zero dropouts anywhere."""
    return [
        pr for pr in records
        if (pr.export_name, pr.game_id) not in dropout_game_ids
    ]
...
def load_human_team_records(verbose: bool = True):
    ...
    Scope: current April exports. Games with any dropout player-round are
    ...
    for pr in clean:
        if pr.round.round_type == "human":
            human_teams[(pr.export_name, pr.game_id, pr.round.round_number)].append(pr)
    ...
    for (export_name, game_id, round_number), team_prs in complete.items():
        ...
        try:
            ...
            continue
        env_cache_key = str(value_dir)
        if env_cache_key not in env_cache:
            env_cache[env_cache_key] = _load_env_config(value_dir)
        ...
        records.append({
            ...
            "env_config": env_cache[env_cache_key],
        })
    ...
    if verbose:
        ...
        print(f"[pipeline] current exports: {len(all_prs)} player-rounds total, "
              f"{len(dropout_games)} export-games excluded as dropout")
