"""Triple-objective × 7-model comparison on all 5 current exports.

For each of 3 objectives (combo_r, agg_ll, mean_ll) × 7 models, fits the
model's params on the clean human team-rounds from the five current
treatment-condition exports under that objective, then evaluates ALL metrics
at the fitted point.
...
def build_table(cells, n_records):
    lines = [
        ...
        "",
        f"Fitted on {n_records} clean human team-rounds from the five current "
        "treatment-condition exports (04-23, 04-27a, 04-27b, 05-11a, 05-11b — "
        "see `pipeline.load_human_team_records`). Every team-round must have "
        "a matching value matrix in `human_envs_value_matrices/"
...
def make_figures(cells):
    ...
    fig.suptitle("Model Ranking Stability Across Objectives (current exports)",
                 fontsize=13)
    fig.tight_layout()
    ...
    if n_panels > 0:
        ...
        for model, pnames in param_models.items():
            for pname in pnames:
                ...
                ax_idx += 1
        fig.suptitle("Fitted Parameters by Objective (current exports)", fontsize=13)
        fig.tight_layout()
    ...
    fig.suptitle("Metric Correlations Across All Fits (current exports)", fontsize=13)
    fig.tight_layout()
...
def main():
    print("=" * 66)
    print("Current-export metric comparison: 3 objectives x 7 models (human-only)")
    print("=" * 66)
    ...
    from pipeline import EXPORT_NAMES
    output = {
