"""Stage 2: Bayesian Walk PS — tunes epsilon_switch.

Stick-or-switch using posterior marginals (no value matrix).
Choice params: epsilon_switch
"""
...
def evaluate(records, trajectories, epsilon_switch):
    for i, rec in enumerate(records):
        rec['_traj_idx'] = i

    def predict_fn(record):
        idx = record['_traj_idx']
        return predict_from_trajectory_walk_ps(trajectories[idx], epsilon_switch)
...
def main():
    ...
    completed = get_completed_keys(results, ['epsilon_switch'])

    es_vals = np.linspace(0.0, 1.0, 21)
    print(f"\nCoarse grid: {len(es_vals)} points")
    print(f"  Already completed: {len(completed)}")

    for es in es_vals:
        if (float(es),) in completed:
            continue
        res = evaluate(records, trajectories, es)
        results.append(res)
        save_checkpoint(coarse_path, results)

    best = pick_best(results, METRIC)
    print(f"Coarse best: eps_switch={best['epsilon_switch']:.4f} combo_r={best['combo_r']:.4f}")

    # Phase 2: Refined
    refined_path = str(CHECKPOINT_DIR / 'refined_results.json')
    refined = load_checkpoint(refined_path)
    completed_r = get_completed_keys(refined, ['epsilon_switch'])

    es_step = 1.0 / 20
    fine_es = np.linspace(max(0.0, best['epsilon_switch'] - es_step),
                          min(1.0, best['epsilon_switch'] + es_step), 11)

    print(f"\nRefined grid: {len(fine_es)} points")
    for es in fine_es:
        if (float(es),) in completed_r:
            continue
        res = evaluate(records, trajectories, es)
        refined.append(res)
    ...
    print(f"\nScipy polish from eps_switch={best['epsilon_switch']:.4f}")
    opt = minimize(objective, [best['epsilon_switch']], method='L-BFGS-B',
