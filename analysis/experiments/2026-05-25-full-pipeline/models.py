"""Model factories for the 2026-05-25 full pipeline.

Seven models, each producing a per-stage ``{predicted_dist, human_combo,
model_marginal}`` record given a precomputed trajectory and Stage-2 params.

Factory pattern:
    make_factory(params) -> factory(records_subset, traj_subset) -> predict_fn(record)

The closure layers let predict_fn resolve a record to its precomputed
trajectory via ``id(record)`` so we don't recompute posteriors during
fitting.
"""
...
# ──────────────────────────────────────────────────────────────────────
# Bayesian Threshold — switch only when another role's EV beats current by δ
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# Mixture-PS — convex combination of walk_ps and thresh_ps
# ──────────────────────────────────────────────────────────────────────
